  ...
  return (
    <div>
      <h2 id="message-heading" data-cy="MessageHeading">
        <Translate contentKey="messageApp.message.home.title">Messages</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="messageApp.message.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/message/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="messageApp.message.home.createLabel">Create new Message</Translate>
          </Link>
        </div>
      </h2>
        ...
        {messageList && messageList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="messageApp.message.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('content')}>
                  <Translate contentKey="messageApp.message.content">Content</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('content')} />
                </th>
                <th className="hand" onClick={sort('ciphertext')}>
                  <Translate contentKey="messageApp.message.ciphertext">Ciphertext</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('ciphertext')} />
                </th>
                <th className="hand" onClick={sort('timestamp')}>
                  <Translate contentKey="messageApp.message.timestamp">Timestamp</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('timestamp')} />
                </th>
                <th>
                  <Translate contentKey="messageApp.message.conversation">Conversation</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="messageApp.message.sender">Sender</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="messageApp.message.home.notFound">No Messages found</Translate>
            </div>
          )
        )}
      </div>
