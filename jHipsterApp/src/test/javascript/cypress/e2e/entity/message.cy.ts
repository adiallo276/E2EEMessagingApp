...
describe('Message e2e test', () => {
  ...
  const messageSample = { content: 'furthermore', timestamp: '2025-11-23T18:13:53.430Z' };
  ...
  describe('new Message page', () => {
    ...
    it('should create an instance of Message', () => {
      cy.get(`[data-cy="content"]`).type('usefully bitterly babushka');
      cy.get(`[data-cy="content"]`).should('have.value', 'usefully bitterly babushka');

      cy.get(`[data-cy="ciphertext"]`).type('../fake-data/blob/hipster.txt');
      cy.get(`[data-cy="ciphertext"]`).invoke('val').should('match', new RegExp('../fake-data/blob/hipster.txt'));

      cy.get(`[data-cy="timestamp"]`).type('2025-11-23T12:00');
      cy.get(`[data-cy="timestamp"]`).blur();
      cy.get(`[data-cy="timestamp"]`).should('have.value', '2025-11-23T12:00');
