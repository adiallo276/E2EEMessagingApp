 ...
 */
public class ConversationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all conversations").get("/api/conversations").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new conversation")
                        .post("/api/conversations")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"createdAt\": \"2020-01-01T00:00:00.000Z\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_conversation_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created conversation").get("${new_conversation_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created conversation").delete("${new_conversation_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Conversation entity").exec(scn);
